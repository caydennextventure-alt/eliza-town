/**
 * Ephemeral HNSW Vector Index for ElizaOS
  ...
  async add(id: string, vector: number[]): Promise<void> {
    ...
    for (let l = Math.min(level, this.maxLevel); l >= 0; l--) {
      ...
      for (const neighbor of selectedNeighbors) {
        ...
        if (neighborNode) {
          let nSet = neighborNode.neighbors.get(l);
          if (!nSet) {
            nSet = new Set();
            neighborNode.neighbors.set(l, nSet);
          }
          nSet.add(id);

          if (nSet.size > M) {
            const toKeep = this.selectBestNeighbors(neighborNode.vector, nSet, M);
