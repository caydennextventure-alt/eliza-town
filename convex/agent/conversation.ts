/**
 * Agent Conversation - ElizaOS Powered
 * 
 * This module handles all agent conversation logic using ElizaOS.
 * Chat responses are generated by the ElizaOS runtime running in Convex.
...
export async function startConversationMessage(
  ...
  // Try ElizaOS first if this agent has an ElizaOS mapping
  if (elizaAgent) {
    try {
      ...
      console.error(`[ElizaOS] Start conversation error, falling back to built-in:`, error);
    }
  }
...
export async function continueConversationMessage(
  ...
  // Try ElizaOS first if this agent has an ElizaOS mapping
  if (elizaAgent && messages.length > 0) {
    const lastMessage = messages[messages.length - 1];

    // Only respond if the last message is from the other player
    if (lastMessage && lastMessage.author === otherPlayerId) {
      try {
        ...
        console.error(`[ElizaOS] Continue conversation error, falling back to built-in:`, error);
      }
    }
  }
  ...
  const memoriesResult = await memory.searchMemories(ctx, player.id as GameId<'players'>, embedding, 3);
  const prompt = [
    `You are ${player.name}, and you're currently in a conversation with ${otherPlayer.name}.`,
    `The conversation started at ${started.toLocaleString()}. It's now ${new Date(now).toLocaleString()}.`,
  ];
  prompt.push(...agentPrompts(otherPlayer, agent, otherAgent ?? null));
  prompt.push(...relatedMemoriesPrompt(memoriesResult));
  prompt.push(
...
export async function leaveConversationMessage(
  ...
  if (elizaAgent) {
    try {
      ...
      console.error(`[ElizaOS] Leave conversation error, falling back to built-in:`, error);
    }
  }
